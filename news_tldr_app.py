...
def api_keys_form():
    with st.form("api_keys_form"):
        newsapi_key = st.text_input("Enter your News API key:", type="password")
        openai_key = st.text_input("Enter your OpenAI API key:", type="password")
        
        submitted = st.form_submit_button("Submit")
        if submitted and newsapi_key and openai_key:
...
def main():
    st.title("News TLDR App")
    
    initialize_session_state()
    
    if not st.session_state.api_keys_set:
        st.warning("Please set your API keys to continue")
        api_keys_form()
    else:
        # Create the search interface
        query = st.text_input("Enter your news search query:")
        num_articles = st.slider("Number of articles to summarize:", 1, 5, 3)
        
        if st.button("Search and Summarize"):
            if query:
                with st.spinner("Searching and summarizing articles..."):
                    result = asyncio.run(run_workflow(query, num_articles_tldr=num_articles))
                    st.markdown(result)
            else:
